...
for line in content.split('\n'):
    ...
    if in_table and line_s.startswith('|'):
        ...
        if len(cols) >= 4:
            # We check if there are 5 columns (including 電脳有名度)
            # Format: | セリフ | 登場巻・話数 | 発言者 | 電脳有名度 | 意味 |
            # If currently 4 columns, we add a default empty rating
            quote_text = cols[0]
            volume = cols[1]
            character = cols[2]
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .col-rating { 
      ...
      white-space: nowrap; 
      font-size: 14px;
      text-shadow: 0 0 10px rgba(215, 173, 74, 0.3);
    }
...
for q in quotes:
    ...
    table_html += f'''
          <tr>
            ...
            <td class="col-rating">{rating_text}</td>
...
print(f"Updated HTML design to TABLE layout with {len(quotes)} quotes.")
